...
class DRType: # DNS Record Type
    A = 1 # IPv4
    NS = 2 # an authoritative name server; not implemented
...
class DnsQuery:
    ...
    def process(self, data):
        ...
        ra = ord(data[3]) >> 7 == 1
        rc = ord(data[3]) & 0xF
        nbAn = (ord(data[6]) << 8) + ord(data[7])
        nbAu = (ord(data[8]) << 8) + ord(data[9])
        nbAd = (ord(data[10]) << 8) + ord(data[11])
        if rc != 0:
            return aa, tc, ra, rc, nbAn, nbAu, nbAd, None, None, None
        else:
            index = len(self.request)
            listAns = listAus = listAds = []
            ...
            for i in listTemp:
                ...
                for j in range(i[1]):
                    ...
                    temp.append(name)
                    # print name
                    temp.append((ord(data[index]) << 8) + ord(data[index + 1]))
                    temp.append((ord(data[index + 2]) << 8) + ord(data[index + 3]))
                    temp.append((ord(data[index + 4]) << 24) + (ord(data[index + 5]) << 16) + (ord(data[index + 6]) << 8) + ord(data[index + 7]))
                    offset = ord(data[index + 8])
                    temp.append(offset)
